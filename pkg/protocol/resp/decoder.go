...
var (
	...
	ErrBadMultiBulkContent = errors.New("bad multi-bulk content, should be bulk bytes")
)
...
func NewDecoder(r io.Reader) *Decoder {
	return NewDecoderBuffer(protocol.NewBufReaderSize(r, 8192))
}

func NewDecoderSize(r io.Reader, size int) *Decoder {
	return NewDecoderBuffer(protocol.NewBufReaderSize(r, size))
}

func NewDecoderBuffer(br *protocol.BufReader) *Decoder {
...
func (d *Decoder) decodeResp() (*Resp, error) {
	...
	switch r.Type {
	default:
		return nil, fmt.Errorf("bad resp type %v", r.Type)
